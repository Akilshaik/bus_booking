...
def add_review_view(request, bus_id):
    ...
    has_booking = Booking.objects.filter(user=request.user, trip__bus=bus, status="CONFIRMED").exists()
    if not has_booking:
...
def bus_reviews_view(request, bus_id):
    bus = get_object_or_404(Bus, id=bus_id)
    reviews = Review.objects.filter(bus=bus).select_related("user").order_by("-created_at")
    return render(request, "reviews/bus_reviews.html", {"bus": bus, "reviews": reviews})